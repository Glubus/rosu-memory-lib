...
impl<'a> UserReader<'a> {
    ...
    pub fn get_user_bancho_status(&mut self) -> Result<i32, Error> {
        match self.osu_type {
            OsuClientKind::Stable => stable::memory::get_user_bancho_status(self.process, self.state),
            _ => Err(Error::Unsupported(
                "Unsupported osu type for now".to_string(),
            )),
        }
    ...
    pub fn get_user_country_code(&mut self) -> Result<i32, Error> {
        match self.osu_type {
            OsuClientKind::Stable => stable::memory::get_user_country_code(self.process, self.state),
            _ => Err(Error::Unsupported(
                "Unsupported osu type for now".to_string(),
            )),
        }
