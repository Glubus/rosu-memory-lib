...
pub fn key_ptr(p: &Process, state: &mut State) -> Result<i32, Error> {
    ...
    // TODO : add mode (osu mania taiko) checking
    Ok(key_ptr)
}

pub fn key_overlay(p: &Process, state: &mut State) -> Result<KeyOverlay, Error> {
    ...
    // TODO : check item size minimum 4
    let key_1_pressed = p.read_i32(p.read_i32(key_array_addr + 0x8)? + 0x1C)?;
    let key_1_count = p.read_i32(p.read_i32(key_array_addr + 0x8)? + 0x14)?;

    let key_2_pressed = p.read_i32(p.read_i32(key_array_addr + 0xc)? + 0x1C)?;
    let key_2_count = p.read_i32(p.read_i32(key_array_addr + 0xc)? + 0x14)?;

    let mouse_1_pressed = p.read_i32(p.read_i32(key_array_addr + 0x10)? + 0x1C)?;
    let mouse_1_count = p.read_i32(p.read_i32(key_array_addr + 0x10)? + 0x14)?;

    let mouse_2_pressed = p.read_i32(p.read_i32(key_array_addr + 0x14)? + 0x1C)?;
