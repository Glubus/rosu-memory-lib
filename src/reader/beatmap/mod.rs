...
impl<'a> BeatmapReader<'a> {
    pub fn new(p: &'a Process, state: &'a mut State, osu_type: OsuClientKind) -> Result<Self, Error> {
        Ok(Self {
            process: p,
            state,
            osu_type,
    ...
    pub fn get_beatmap_drain_time(&mut self) -> Result<i32, Error> {
        match self.osu_type {
            OsuClientKind::Stable => stable::memory::get_beatmap_drain_time(self.process, self.state),
            _ => Err(Error::Unsupported(
                "Unsupported osu type for now".to_string(),
            )),
        }
    ...
    pub fn get_beatmap_object_count(&mut self) -> Result<u32, Error> {
        match self.osu_type {
            OsuClientKind::Stable => stable::memory::get_beatmap_object_count(self.process, self.state),
            _ => Err(Error::Unsupported(
                "Unsupported osu type for now".to_string(),
            )),
        }
    ...
    pub fn get_beatmap_slider_count(&mut self) -> Result<i32, Error> {
        match self.osu_type {
            OsuClientKind::Stable => stable::memory::get_beatmap_slider_count(self.process, self.state),
            _ => Err(Error::Unsupported(
                "Unsupported osu type for now".to_string(),
            )),
        }
    ...
    pub fn get_beatmap_star_rating(&mut self) -> Result<BeatmapStarRating, Error> {
        match self.osu_type {
            OsuClientKind::Stable => stable::file::get_beatmap_star_rating(self.process, self.state),
            _ => Err(Error::Unsupported(
                "Unsupported osu type for now".to_string(),
            )),
        }
